...
export async function loadContext(sessionId) {
    ...
    return {
        last_restaurant_id: st.last_restaurant_id,
        last_menu_at: st.last_menu_at,
        cart_item_count: cart?.items_snapshot?.length ?? 0,
        active_order_id: ordersList.orders[0]?.id,
        default_lat: st.default_lat,
        default_lon: st.default_lon,
    };
}
export async function resolveIntent(message, action, params, context) {
    if (action && params) {
        return {
            name: action,
            confidence: 1,
    ...
    if (message)
        return resolveWithLLM(message, context);
    return { name: "fallback", confidence: 0, resolved_params: {} };
}
export async function runPipeline(input) {
    const context = await loadContext(input.sessionId);
    const intent = await resolveIntent(input.message, input.action, input.params, context);

...
const VALID_INTENTS = new Set(INTENTS);
const CLASSIFY_INTENT_TOOL = {
    type: "function",
    function: {
        name: "classify_intent",
        description: "Classify the user's intent for a food ordering assistant (Zomato). Use conversation context to fill restaurant_id, order_id, lat, lon when the user implies 'this restaurant' or 'my order' without saying the id.",
        parameters: {
            type: "object",
            properties: {
                intent: {
                    type: "string",
                    enum: [...INTENTS],
                    description: "One of: discover_restaurants, view_menu, add_to_cart, view_cart, update_cart_item, clear_cart, place_order, track_order, fallback",
                },
                confidence: {
                    type: "number",
                    minimum: 0,
                ...
                resolved_params: {
                    type: "object",
                    description: "Extracted parameters: query (food/cuisine/location), lat, lon, restaurant_id, item_name, quantity, order_id, line_id as needed for the intent",
                    properties: {
                        query: { type: "string" },
                        lat: { type: "number" },
                        lon: { type: "number" },
...
function parseFunctionArguments(args) {
    try {
        ...
        if (!name || !VALID_INTENTS.has(name))
            return null;
        const confidence = typeof parsed.confidence === "number"
            ? Math.max(0, Math.min(1, parsed.confidence))
            : 0.9;
        const resolved_params = parsed.resolved_params && typeof parsed.resolved_params === "object"
 ...
 */
export async function resolveWithLLM(message, context) {

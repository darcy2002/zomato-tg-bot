...
export async function placeOrder(input) {
    ...
    const itemsSnapshot = (cart.items_snapshot ?? []).map((i) => ({
        item_id: i.item_id,
        name: i.name,
        quantity: i.quantity,
    ...
    const estimatedAt = mcpResult.estimated_delivery_at
        ...
        : null;
    const order = await ordersRepo.create(input.session_id, input.user_id, input.delivery_address, itemsSnapshot, totalCents, cart.currency, input.idempotency_key ?? null, mcpResult.order_id, estimatedAt);
...
export async function getOrderStatus(sessionId, orderId) {
...
export async function listOrders(sessionId, limit, offset) {

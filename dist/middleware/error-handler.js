...
export function errorHandler(err, _request, reply) {
    ...
    if (err.validation) {
        reply.status(400).send(formatErrorResponse(ErrorCodes.VALIDATION_ERROR, err.message, requestId, err.validation?.map((v) => ({
            path: v.instancePath,
            message: v.message,
        }))));
        return;
    ...
    reply
        ...
        .send(formatErrorResponse(ErrorCodes.INTERNAL_ERROR, "Internal server error", requestId));
}

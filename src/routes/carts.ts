...
export default async function cartsRoutes(app: FastifyInstance) {
  app.get<{ Headers: { "x-session-id"?: string } }>(
    "/carts/current",
    { schema: getCurrentCart },
    async (request, reply) => {
  ...
  }>("/carts/items", { schema: postCartItems }, async (request, reply) => {
  ...
  app.patch<{
    ...
    Body: { session_id: string; quantity: number };
  }>("/carts/items/:lineId", { schema: patchCartItem }, async (request, reply) => {
    const session = await sessionsRepo.getById(request.body.session_id);
    if (!session) throw new AppError(ErrorCodes.SESSION_NOT_FOUND, "Session not found", 404);
  ...
  app.delete<{ Headers: { "x-session-id"?: string } }>(
    "/carts/current",
    async (request, reply) => {
      const sessionId = request.headers["x-session-id"];
      if (!sessionId) throw new AppError(ErrorCodes.UNAUTHORIZED, "X-Session-Id required", 401);

...
function buildInlineKeyboard(suggestedActions: { id: string; label: string; action: string; params: Record<string, unknown> }[], sessionId: string) {
...
async function sendConversationReply(
  ctx: Context,
  replyText: string,
  suggestedActions: { id: string; label: string; action: string; params: Record<string, unknown> }[],
  sessionId: string
) {
